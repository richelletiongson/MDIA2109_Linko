...
const ENJOY = [
  "Discussing music deeply", "Casual listener", "Both"
];
...
export default function Questionaire07Page() {
    ...
    const toggleEnjoy = (enjoy) => {
        setSelected(selected =>
            selected.includes(enjoy)
                ? selected.filter(e => e !== enjoy)
                : [...selected, enjoy]
        );
    };
    ...
    return (
        <div className={styles.page}>
            ...
            <h2 className={styles.question}>
                When it comes to music, what do you prefer?<br />
            ...
            <div className={styles.languageGrid}>
                {ENJOY.map(enjoy => (
                    <Button
                        key={enjoy}
                        buttonText={enjoy}
                        type={selected.includes(enjoy) ? 'green' : 'white'}
                        size="big_pill"
                        onClick={() => toggleEnjoy(enjoy)}
